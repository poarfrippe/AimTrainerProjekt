...
const camera = new THREE.PerspectiveCamera(localStorage.getItem('FOV'), window.innerWidth / window.innerHeight, 0.1, 500);
...
const sensiy = localStorage.getItem('sensiy')
const sensix = localStorage.getItem('sensix')
...
function sendScore() {
    ...
    fetch("http://89.107.108.231:18787/flick", {
        method: "post",
...
function handleMouseMove(event) {
    ...
    if(document.pointerLockElement == document.body && startsec > 0) {
        ...
        //console.log("sepplfunctiopn sog: " + checkNumberSperegeometrys())
        if (checkNumberSperegeometrys() == 0) {
...
function drawline(points, color = 0x000000) {
...
function inittargets(howmanny) {
    ...
    for (let i = 0; i < howmanny; ++i) {
        ...
        mesh.position.x = ((Math.random() - 0.5) * 10000) % (roomwidth/2-TARGETSIZE);
        mesh.position.y = ((Math.random() - 0.5) * 10000) % (roomheight/2-TARGETSIZE);
        mesh.position.z = ((Math.random()) * -10000) % (roomdepth-TARGETSIZE);
                    ...
                    if ((newcords.x < scene.children[i].position.x+TARGETSIZE && newcords.x > scene.children[i].position.x-TARGETSIZE) && 
                        (newcords.y < scene.children[i].position.y+TARGETSIZE && newcords.y > scene.children[i].position.y-TARGETSIZE)) {
                            if (startsec > 0) {
                                ...
                                ++score
                                new Audio("../sounds/hitsoundCut.mp3").play()
                                settextscore("Score: " + score)
                                if (checkNumberSperegeometrys() == 0) {
                        ...
                        new Audio("../sounds/misssoundCut.mp3").play()
                    }
                }
            }
        }
...
function checkNumberSperegeometrys() {
    let number = 0
    for(let i = 0; i < scene.children.length; ++i){
        if (scene.children[i].type == "Mesh") {
            if(scene.children[i].geometry.type == "SphereGeometry") {
